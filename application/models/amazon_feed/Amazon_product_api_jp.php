...
class Amazon_product_api_jp extends CI_Model
{
  private $seller_id='';
  private $auth_token='';
  private $access_key='';
  private $secret_key='';
  private $market_id='';
  private $service_url='mws.amazonservices.jp';
...
public function fetch_product_detail($user_id,$asin)
  { 
    try
    {
       ...
       if(isset($res->GetLowestOfferListingsForASINResult[0]->Product))
      {
			...
			$payload['low_price']= (((string)$res->GetLowestOfferListingsForASINResult[0]->Product->LowestOfferListings->LowestOfferListing->Price->LandedPrice->Amount)/106.72);
 ...
 private function built_query_string($add_param)
 {
        ...
        sort($url_parts);
            ...
            $url = "https://mws.amazonservices.jp/Products/2011-10-01?". $url_string . "&Signature=" . $signature;
            return $url; 
 }
}