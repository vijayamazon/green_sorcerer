...
class Amazon_product_api extends CI_Model
{
 ...
 //echo "response:$res";
      ...
      $payload=[];
      $payload['lm_ean']=$payload['lm_asin']=$payload['image']=$payload['sales_rank']=$payload['weight']=$payload['asin_counts']=$payload['title']=$payload['list_price']='';
      ...
      if(isset($res->GetMatchingProductForIdResult[0]->Products->Product))
      {
            ...
            if(isset($res->GetMatchingProductForIdResult->Products->Product->AttributeSets))
            {
              ...
              $payload['image']=isset($ns->ItemAttributes->SmallImage->URL)?str_replace('_SL75_','_SL500_',(string)$ns->ItemAttributes->SmallImage->URL):'';
               $payload['weight']=isset($ns->ItemAttributes->ItemDimensions->Weight)?(string)$ns->ItemAttributes->ItemDimensions->Weight:'';
  ...
  { 
    try
    {
       ...
       if(isset($res->GetLowestOfferListingsForASINResult[0]->Product))
      {
			...
			$payload['low_price']= (string)$res->GetLowestOfferListingsForASINResult[0]->Product->LowestOfferListings->LowestOfferListing->Price->LandedPrice->Amount;
			
            }
			 if(preg_match_all('/<LowestOfferListing><Qualifiers><ItemCondition>New<\/ItemCondition><ItemSubcondition>[^>]*?<\/ItemSubcondition><FulfillmentChannel>Amazon<\/FulfillmentChannel><ShipsDomestically>[^>]*?<\/ShipsDomestically><ShippingTime><Max>[^>]*?<\/Max><\/ShippingTime><SellerPositiveFeedbackRating>[^>]*?<\/SellerPositiveFeedbackRating><\/Qualifiers><NumberOfOfferListingsConsidered>[^>]*?<\/NumberOfOfferListingsConsidered><SellerFeedbackCount>[^>]*?<\/SellerFeedbackCount><Price><LandedPrice><CurrencyCode>USD<\/CurrencyCode><Amount>([^>]*?)<\/Amount><\/LandedPrice><ListingPrice><CurrencyCode>USD<\/CurrencyCode><Amount>[^>]*?<\/Amount><\/ListingPrice><Shipping><CurrencyCode>USD<\/CurrencyCode><Amount>[^>]*?<\/Amount><\/Shipping><\/Price><MultipleOffersAtLowestPrice>[^>]*?<\/MultipleOffersAtLowestPrice><\/LowestOfferListing>/i',$response,$matches)){
	$res=$matches[1];
	 //print_r($res[0]);
          //print_r($matches);  
        $payload['low_new_price']=$res[0];
