...
class Amazon_mws_product_api_in extends CI_Controller 
{
  public function  __construct()
  {
	     parent::__construct();
       	 $this->load->model('amazon_feed/Amazon_product_api_in','amazon_api');
  }

  public function product_match()
  {
    $query=$this->db->query("SELECT * FROM amazon_profile WHERE profile_id='5'");
      ...
      foreach($user as $usr)
      {
         ...
         if(!empty($prod_list))
         {
 			...
 			//time_nanosleep(0, 500000000);
 			  	...
 			  	{
 			  		$qi="UPDATE product_info SET pro_in_rank=".$this->db->escape($res['payload']['sales_rank'])." WHERE pro_isbn='".$prd['pro_isbn']."' ";
					//print_r($qi);
 			 ...
 			 $res=$this->amazon_api->fetch_product_detail($usr['profile_id'],$res['payload']['lm_asin']);
 			  	if($res['status_code']==1)
 			  	{
 			  		$qi="UPDATE product_info SET pro_in_price=".$this->db->escape($res['payload']['low_price'])." WHERE pro_isbn='".$prd['pro_isbn']."' ";
					//print_r($qi);
